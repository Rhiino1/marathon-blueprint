...
const racePlayers = nodecg.Replicant('players');
const tokenLinks = nodecg.Replicant('tokenLinks');
const finishedTimes = nodecg.Replicant('finishedTimes');
...
timerReplicant.on('change', value =>{
	timer.hours = value.hours;
	hours = value.hours;
	timer.minutes = value.minutes;
	minutes = value.minutes;
	timer.seconds = value.seconds;
	seconds = value.seconds;
	timer.tenthseconds = value.tenthseconds;
	tenthseconds = value.tenthseconds;
})

async function chronometer() {
	// console.log(typeof timer.hours, typeof timer.minutes, typeof timer.seconds);
	if(timer.hours !== `00` && timer.minutes !== `00` && timer.seconds !== `00` && timer.tenthseconds !== `0`){
...
nodecg.listenFor('doneRacePlayer', (name) => {
	...
	finishedTimes.value.push({name: name, time: text, pos: racePlayers.value.pos})
