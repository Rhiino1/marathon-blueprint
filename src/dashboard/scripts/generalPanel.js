...
NodeCG.waitForReplicants(generalRunInfo).then(() => {
  generalRunInfo.on('change', (value) => {
    ...
    generalText[4].value = value.coms;
...
generalRunButtons[1].addEventListener('click', (e) => {
  ...
  generalRunInfo.value.coms = generalText[4].value;
  // generalRunInfo.value.year = generalText[4].value;
  for (let i = 0; i < playersNames.length; i++) {
    // console.log(racePlayersName.value.players[i], playersNames[i].value)
    // console.log(racePlayersName.value.twitch[i], playersLabels[i].value)
